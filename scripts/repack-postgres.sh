...
pushd $PACKDIR/pgsql
tar cJf $RSRC_DIR/postgresql-$VERSION-linux-x86_64.txz \
  share/postgresql \
  lib \
  bin/initdb \
  bin/pg_ctl \
...
pushd $PACKDIR/pgsql
tar cJf $RSRC_DIR/postgresql-$VERSION-darwin-x86_64.txz \
  share/postgresql \
...
pushd $PACKDIR/pgsql
tar cJf $RSRC_DIR/postgresql-$VERSION-windows-x86_64.txz \
  share \
  lib/iconv.lib \
  lib/libxml2.lib \
  lib/ssleay32.lib \
